...
사용은
	...
	<!--paginate-->
	<div class="paginate">
		<a class="pre_end" href="#">처음</a> <a class="pre" href="#">이전</a> <a
			href="#">1</a> <strong>2</strong> <a href="#">3</a> <a href="#">4</a>
		<a href="#">5</a> <a href="#">6</a> <a href="#">7</a> <a href="#">8</a>
		<a href="#">9</a> <a href="#">10</a> <a class="next" href="#">다음</a>
	...
	$.fn.SHPaging = function(options) {
		...
		return  this.each(function(){
			...
			if( opt.display_skip ){ // 이전 스킵
				if(opt.current_page < opt.rows_per_page +1) { 
					...
					var pp = parseInt((opt.current_page -1)/ opt.rows_per_page )*opt.rows_per_page; 
					 outbuff += "<a class='pre_end' href=\"javascript:location.replace('"+opt.page_link+pp+"');void(0);\">처음</a>" ;
				} 
			}

			if( opt.display_next ){ // 이전
			   if(opt.current_page>1){
				 outbuff +=  "<a class='pre' href=\"javascript:location.replace('"+opt.page_link+( opt.current_page -1)+"');void(0);\">이전</a>" ;
			   }else{
				 outbuff +=  "<a class='pre' href='#'>이전</a>" ; 
			   }
			}
			...
			for(var jn = st1; jn < st2; jn++){
				if ( jn <= total_page ){
					outbuff += (jn == opt.current_page)? "<strong>"+jn+"</strong>":"<a href=\"javascript:location.replace('"+opt.page_link+jn+"');void(0);\">"+jn+"</a>";
				}
			}
			//outbuff +="</ol></div>";
			...
			if( opt.display_skip ){ // 다음 스킵
				if( total_page < opt.rows_per_page  || total_page < jn) { 
					...
					var np = jn; 
					outbuff += "<a class='next' href=\"javascript:location.replace('"+opt.page_link+np+"');void(0);\">다음</a>"; 
				}
			}

			if( opt.display_next ){// 다음 
			   if(  opt.current_page  < total_page){
				 outbuff += "<a class='next_end' href=\"javascript:location.replace('"+opt.page_link+(opt.current_page+1)+"');void(0);\">끝</a>";
			   }else{
